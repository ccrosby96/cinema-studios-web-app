...
function PeopleListItem (person = {
    ...
    const peopleData = person.person
    const url = "https://image.tmdb.org/t/p/w500";
    let profile_path = url + peopleData.profile_path;
    const title = peopleData.name;
    const department = peopleData.known_for_department;

    return (
        <div className ="list-group-item m-1">
            <div className="row">
                <div className="col-4">
                    <img className="card-img rounded-3 movie-list-item-img m-0 p-0" src= {profile_path} alt="Image Not Found"/>
                </div>
                <div className="col-8 m-0 p-0">
                    <div className="m-0 p-0">
                        <h5 className="mb-0 pb-0">{title}</h5>
                        <span className = "grey-text mt-0 pt-0">Known for {department}</span>
                    </div>
                </div>
            </div>
        </div>
